...
class AudioConfig:
    ...
    def get_stt_model_path(self) -> str:
        ...
        with importlib.resources.files("personalparakeet.models").joinpath(
            "stt_model.onnx"
        ) as path:
            return str(path)
...
class V3Config:
    ...
    def _update_from_dict(self, data: Dict[str, Any]):
        ...
        if "audio" in data:
            ...
            self.audio.silence_threshold = audio_data.get(
            ...
            self.audio.stt_audio_threshold = audio_data.get(
                "stt_audio_threshold", self.audio.stt_audio_threshold
            )

        # Update VAD config
        if "vad" in data:
            vad_data = data["vad"]
            self.vad.silence_threshold = vad_data.get(
                "custom_threshold", self.vad.silence_threshold
            )
            self.vad.pause_threshold = (
                vad_data.get("pause_duration_ms", 1500) / 1000.0
            )  # Convert ms to s
        ...
        if "thought_linking" in data:
            ...
            self.thought_linking.enabled = thought_linking_data.get(
                "enabled", self.thought_linking.enabled
            )
            self.thought_linking.similarity_threshold = thought_linking_data.get(
                "similarity_threshold", 0.3
            )
            self.thought_linking.timeout_threshold = thought_linking_data.get(
                "timeout_threshold", 30.0
            )
            self.thought_linking.cursor_movement_threshold = thought_linking_data.get(
    ...
    def update_vad_settings(
        self, silence_threshold: Optional[float] = None, pause_threshold: Optional[float] = None
    ):
        """Update VAD settings"""
...
def create_balanced_profile() -> ConfigurationProfile:
    ...
    return ConfigurationProfile(
        ...
        audio=AudioConfig(sample_rate=16000, chunk_size=8000, silence_threshold=0.01),
        vad=VADConfig(silence_threshold=0.01, pause_threshold=1.5, frame_duration=0.03),
        clarity=ClarityConfig(enabled=True, rule_based_only=True, target_latency_ms=50.0),
        window=WindowConfig(
            default_width=600,
            default_height=150,
            opacity=0.95,
...
def create_accurate_document_profile() -> ConfigurationProfile:
    ...
    return ConfigurationProfile(
        ...
        clarity=ClarityConfig(
            enabled=True, rule_based_only=False, target_latency_ms=100.0  # Full clarity processing
        ),
        window=WindowConfig(
            default_width=600,
            default_height=150,
...
def create_low_latency_profile() -> ConfigurationProfile:
    ...
    return ConfigurationProfile(
        ...
        audio=AudioConfig(
            sample_rate=16000, chunk_size=2000, silence_threshold=0.03  # Very small chunks
        ),
        vad=VADConfig(
        ...
        clarity=ClarityConfig(
            enabled=False, rule_based_only=True, target_latency_ms=20.0  # Disable clarity for speed
        ),
        window=WindowConfig(
            default_width=600,
            default_height=150,
...
class ProfileManager:
    ...
    def list_available_profiles(self) -> List[str]:
        ...
        standard_profiles = ["Fast Conversation", "Balanced", "Accurate Document", "Low-Latency"]
