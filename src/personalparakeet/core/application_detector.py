...
class ApplicationProfile:
    ...
    preferred_strategies: List[str] = field(
        default_factory=lambda: ["keyboard", "clipboard", "win32_sendinput"]
    )
...
class EnhancedApplicationDetector:
    ...
    def _init_linux_detection(self):
        ...
        logger.info(
            f"Linux detection - xprop: {self.has_xprop}, xdotool: {self.has_xdotool}, qdbus: {self.has_qdbus}"
        )
    ...
    def detect_current_application(self) -> Optional[ApplicationInfo]:
        ...
        if (
            ...
            and current_time - self.last_detection_time < self.cache_timeout
        ):
            ...
            if process_handle:
                try:
                    ...
                    if self.kernel32.QueryFullProcessImageNameW(
                        process_handle, 0, name_buffer, ctypes.byref(name_size)
                    ):
    ...
    def _detect_linux_application(self) -> Optional[ApplicationInfo]:
        ...
        try:
            ...
            if self.has_xprop:
                result = subprocess.run(
                    ["xprop", "-id", window_id], capture_output=True, text=True, timeout=2
                )
                if result.returncode != 0:
                    return None
    ...
    def _detect_macos_application(self) -> Optional[ApplicationInfo]:
        ...
        try:
            ...
            title_result = subprocess.run(
                ["osascript", "-e", title_script], capture_output=True, text=True, timeout=2
            )
    ...
    def _classify_application(
        self, process_name: str, window_title: str = "", window_class: str = ""
    ) -> ApplicationType:
        """
