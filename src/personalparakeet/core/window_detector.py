...
class WindowDetector:
    ...
    def _init_windows(self):
        ...
        try:
            ...
            logger.warning(
                "pywin32 and psutil not installed. Run 'pip install pywin32 psutil' for window detection on Windows."
            )
            self.enabled = False
    ...
    def _init_linux(self):
        ...
        try:
            ...
            logger.warning(
                "python-xlib and ewmh not installed. Run 'pip install python-xlib ewmh' for window detection on Linux."
            )
            self.enabled = False
        except Exception as e:
    ...
    def _init_macos(self):
        ...
        try:
            ...
            logger.warning(
                "pyobjc-framework-Cocoa not installed. Run 'pip install pyobjc-framework-Cocoa' for window detection on macOS."
            )
            self.enabled = False
    ...
    def _get_window_macos(self) -> Optional[WindowInfo]:
        ...
        try:
            ...
            for window in window_list:
                if window.get("kCGWindowOwnerPID") == active_app.get(
    ...
    def has_window_changed(self) -> bool:
        ...
        changed = (
            current.process_name != self._last_window_info.process_name
            or current.window_title != self._last_window_info.window_title
        )

        if changed:
            logger.debug(
                f"Window changed: {self._last_window_info.process_name} -> {current.process_name}"
            )
            self._last_window_info = current
