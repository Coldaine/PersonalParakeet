        ...
        try:
            ...
            if model_path and Path(model_path).exists():
                ...
                self.model = nemo_asr.models.ASRModel.from_pretrained(
                    "nvidia/parakeet-tdt-1.1b", map_location=self.device
                )
    ...
    def _transcribe_sync(self, audio_chunk: np.ndarray) -> Optional[str]:
        ...
        try:
            ...
            with torch.inference_mode():
                ...
                logger.debug(
                    f"Audio chunk shape: {audio_chunk.shape}, dtype: {audio_chunk.dtype}, max: {np.max(np.abs(audio_chunk)):.4f}"
                )
                ...
                if self.transcription_count % 50 == 0:
                    ...
                    recent_avg = sum(self.transcription_times[-10:]) / min(
                        10, len(self.transcription_times)
                    )
                    logger.info(
                        f"STT performance - Count: {self.transcription_count}, "
