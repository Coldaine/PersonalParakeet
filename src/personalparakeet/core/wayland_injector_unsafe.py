...
if command -v ydotool &> /dev/null; then
    ydotool type '{text.replace("'", "'\"'\"'")}'
elif command -v wtype &> /dev/null; then
    wtype '{text.replace("'", "'\"'\"'")}'
elif command -v xdotool &> /dev/null; then
    export DISPLAY=:0
    xdotool type '{text.replace("'", "'\"'\"'")}'
fi
''')
                script_path = f.name
    ...
    def _inject_clipboard_aggressive(self, text: str) -> bool:
        ...
        try:
            ...
            try:
                self.last_clipboard = subprocess.check_output(
                    ['wl-paste'], text=True, timeout=1
                )
            except:
                self.last_clipboard = ""
