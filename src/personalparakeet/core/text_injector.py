...
class TextInjector:
    ...
    def inject_text(
        self,
        text: str,
        method: InjectionMethod = InjectionMethod.PRIMARY,
        app_info: Optional[ApplicationInfo] = None,
    ) -> bool:
        """
        ...
        context = InjectionContext(
            text=text.strip(), method=method, app_info=app_info or self._get_current_app_info()
        )
    ...
    def _perform_injection(self, context: InjectionContext) -> bool:
        ...
        if (
            ...
            and hasattr(self._enhanced_manager, "inject_text")
    ...
    def _inject_linux_wayland(self, context: InjectionContext) -> bool:
        ...
        try:
            ...
            if not hasattr(self, "_wayland_logged"):
                logger.info(
                    f"Wayland compositor: {self._wayland_injector.capabilities.compositor.value}"
                )
                logger.info(
                    f"Available Wayland methods: {[m.value for m in self._wayland_injector.capabilities.available_methods]}"
                )
    ...
    def _inject_linux_clipboard(self, context: InjectionContext) -> bool:
        ...
        try:
            ...
            try:
                ...
                try:
                    subprocess.run(
                        ["xdotool", "key", "ctrl+v"], check=True, timeout=2, capture_output=True
                    )
    ...
    def _inject_macos_clipboard(self, context: InjectionContext) -> bool:
        ...
        try:
            ...
            try:
                ...
                subprocess.run(
                    ["osascript", "-e", applescript], check=True, timeout=2, capture_output=True
                )
