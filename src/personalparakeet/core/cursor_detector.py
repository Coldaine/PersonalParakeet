...
class CursorDetector:
    ...
    def _init_windows(self):
        ...
        try:
            ...
            logger.warning(
                "pywin32 not installed. Run 'pip install pywin32' for cursor detection on Windows."
            )
            self.enabled = False
    ...
    def _init_linux(self):
        ...
        try:
            ...
            logger.warning(
                "python-xlib not installed. Run 'pip install python-xlib' for cursor detection on Linux."
            )
            self.enabled = False
        except Exception as e:
    ...
    def _init_macos(self):
        ...
        try:
            ...
            logger.warning(
                "pyobjc-framework-Quartz not installed. Run 'pip install pyobjc-framework-Quartz' for cursor detection on macOS."
            )
            self.enabled = False
    ...
    def check_significant_movement(self) -> Tuple[bool, float]:
        ...
        if self._last_position is None:
            self._last_position = CursorPosition(
                x=current_pos[0], y=current_pos[1], timestamp=time.time()
            )
            return False, 0.0
