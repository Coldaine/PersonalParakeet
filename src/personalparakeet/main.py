...
if os.environ.get("PERSONALPARAKEET_BACKGROUND"):
    ...
    handlers = [logging.FileHandler(log_dir / "personalparakeet.log", mode="a")]
else:
...
class PersonalParakeetV3:
    ...
    async def initialize(self, rust_ui):
        ...
        try:
            ...
            self.audio_engine.on_pause_detected = lambda: rust_ui.update_status(
                "Pause detected", "yellow"
            )
    ...
    async def emergency_cleanup(self):
        ...
        try:
            ...
            try:
                ...
                subprocess.run([sys.executable, cleanup_script], timeout=10, capture_output=True)
    ...
    def emergency_cleanup_sync(self):
        ...
        try:
            ...
            subprocess.run([sys.executable, str(cleanup_script)], timeout=5, capture_output=True)
        except Exception as e:
