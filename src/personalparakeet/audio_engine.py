...
class AudioEngine:
    ...
    def __init__(self, config: V3Config, event_loop=None):
        ...
        self.stt_buffer_max_samples = int(
            self.stt_buffer_duration * 16000
    ...
    async def initialize(self):
        ...
        try:
            ...
            if (
                self.config.audio.enable_resampling
                and self.config.audio.capture_sample_rate != self.config.audio.model_sample_rate
            ):
                ...
                logger.info(
                    f"Resampler initialized: {self.config.audio.capture_sample_rate}Hz -> {self.config.audio.model_sample_rate}Hz"
                )

            # Update STT buffer size based on actual model sample rate
            self.stt_buffer_max_samples = int(
                self.stt_buffer_duration * self.config.audio.model_sample_rate
            )
            logger.info(
                f"STT buffer configured: {self.stt_buffer_duration}s = {self.stt_buffer_max_samples} samples at {self.config.audio.model_sample_rate}Hz"
            )
    ...
    async def start(self):
        ...
        self.audio_stream = sd.InputStream(
            ...
            blocksize=int(
                ...
                * self.config.audio.capture_sample_rate
                / self.config.audio.model_sample_rate
            ),
        )
            ...
            try:
                ...
                if not self.audio_queue.full():
                    ...
                    if current_queue_size > 40:  # 80% capacity
                        logger.warning(
                            f"High queue usage: {current_queue_size}/50 chunks ({current_queue_size/50*100:.1f}%)"
                        )
                else:
                    self.queue_overflow_count += 1
                    logger.warning(
                        f"Audio queue full, dropping chunk (overflow #{self.queue_overflow_count}, total received: {self.total_chunks_received})"
                    )

                    # Log detailed queue statistics
                    logger.info(
                        f"Queue stats - Size: {self.audio_queue.qsize()}/50, Overflow rate: {self.queue_overflow_count/self.total_chunks_received*100:.2f}%"
                    )
        ...
        while self.is_listening:
            try:
                ...
                if len(self.stt_buffer) >= self.stt_buffer_max_samples:
                    ...
                    stt_chunk = np.array(
                        self.stt_buffer[: self.stt_buffer_max_samples], dtype=np.float32
                    )
                    ...
                    if max_level >= self.config.audio.silence_threshold:
                        ...
                        logger.debug(
                            f"STT processed {len(stt_chunk)/self.config.audio.model_sample_rate:.1f}s audio in {stt_processing_time:.3f}s"
                        )
                        if stt_processing_time > 1.0:  # 1s threshold for 4s audio
                            logger.warning(
                                f"Slow STT processing: {stt_processing_time:.3f}s for {len(stt_chunk)/self.config.audio.model_sample_rate:.1f}s audio"
                            )
                ...
                if self.total_chunks_processed % 100 == 0:
                    ...
                    logger.info(
                        f"Performance summary - Chunks: {self.total_chunks_processed}, "
                ...
                if (
                    len(self.stt_buffer) > self.config.audio.model_sample_rate * 0.5
                ):  # > 0.5 seconds
                    ...
                    if max_level >= self.config.audio.silence_threshold:
                        ...
                        if text and text.strip():
                            self._handle_transcription(text)
                        logger.debug(
                            f"Processed remaining {len(stt_chunk)/self.config.audio.model_sample_rate:.1f}s audio from buffer"
                        )
        ...
        try:
            if self.stt_processor:
                ...
                if elapsed > 0.3:  # 300ms threshold
                    logger.warning(
                        f"Slow STT processing: {elapsed:.3f}s for chunk shape {audio_chunk.shape}"
                    )
    ...
    def _handle_transcription(self, text: str):
        ...
        if self.on_raw_transcription:
            try:
                ...
                if hasattr(
                    self.on_raw_transcription, "__call__"
                ) and not asyncio.iscoroutinefunction(self.on_raw_transcription):
                    self.on_raw_transcription(text)
                elif asyncio.iscoroutinefunction(self.on_raw_transcription):
                    asyncio.run_coroutine_threadsafe(
                        self.on_raw_transcription(text), asyncio.get_event_loop()
                    )
                else:
                    self.on_raw_transcription(text)
            except Exception as e:
        ...
        if (
            ...
            and self.clarity_engine
            and hasattr(self.clarity_engine, "is_initialized")
            and self.clarity_engine.is_initialized
        ):
                ...
                class CorrectionResult:
                    def __init__(self, original, corrected):
                        ...
                        self.corrections_made = (
                            [(original, corrected)] if original != corrected else []
                        )
        ...
        if self.on_corrected_transcription:
            try:
                if asyncio.iscoroutinefunction(self.on_corrected_transcription):
                    asyncio.run_coroutine_threadsafe(
                        self.on_corrected_transcription(result), asyncio.get_event_loop()
                    )
                else:
        ...
        if self.on_vad_status:
            try:
                if asyncio.iscoroutinefunction(self.on_vad_status):
                    asyncio.run_coroutine_threadsafe(
                        self.on_vad_status(vad_status), asyncio.get_event_loop()
                    )
                else:
