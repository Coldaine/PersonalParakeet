...
from personalparakeet.core.wayland_injector import WaylandInjector, WaylandCompositor, InjectionMethod
...
class TestWaylandInjection:
    ...
    def test_wayland_detection(self):
        ...
        if session_type == 'wayland' or wayland_display:
            ...
            print(f"  Available methods: {[m.value for m in injector.capabilities.available_methods]}")
    
    @pytest.mark.integration
    @pytest.mark.skipif(
        os.environ.get('XDG_SESSION_TYPE') != 'wayland',
        reason="Not running on Wayland"
    )
    def test_wayland_injection_methods(self):
        ...
        assert len(injector.capabilities.available_methods) > 0, \
            "No injection methods available on Wayland"
        
        # Test method priority
        assert len(injector.method_priority) > 0, \
            "No injection methods prioritized"
