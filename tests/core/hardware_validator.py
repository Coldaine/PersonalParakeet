...
class HardwareValidator:
    ...
    def validate_all(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "windows": self.validate_windows() if platform.system() == "Windows" else {"available": False}
        }
    ...
    def generate_report(self) -> str:
        ...
        if validation["audio"]["available"]:
            for device in validation["audio"]["devices"]:
                report.append(f"  - {device['name']} ({device['channels']} channels, {device['sample_rate']}Hz)")
        else:
