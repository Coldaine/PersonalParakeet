...
class ThoughtLinkingIntegration:
    ...
    async def process_text_with_linking(self, text: str) -> InjectionContext:
        ...
        if self.enabled and signals:
            ...
            logger.debug(f"Linking decision: {decision.value} (strongest signal: {strongest.description})")
    ...
    def _create_injection_context(self, text: str, decision: LinkingDecision, 
                                signals: List[LinkingSignal]) -> InjectionContext:
...
def create_thought_linking_integration(thought_linker: ThoughtLinker, 
                                     injection_manager: Any) -> ThoughtLinkingIntegration:
