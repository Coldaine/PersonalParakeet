...
class EnhancedWindowsTextInjector:
    ...
    def inject(self, text: str, strategy_order: Optional[List[str]] = None, 
               app_info: Optional[ApplicationInfo] = None) -> InjectionResult:
        """
    ...
    def _get_optimized_strategy_order(self, app_info: Optional[ApplicationInfo]) -> List[InjectionStrategy]:
    ...
    def _inject_enhanced_clipboard(self, text: str, app_info: Optional[ApplicationInfo] = None) -> bool:
    ...
    def _inject_win32_sendinput(self, text: str, app_info: Optional[ApplicationInfo] = None) -> bool:
...
class EnhancedInjectionManager:
    ...
    def inject_text(self, text: str, app_info: Optional[ApplicationInfo] = None) -> bool:
        ...
        with self.injection_lock:
            try:
                ...
                if result.success:
                    logger.info(f"✓ Injection #{self.injection_count} successful with {result.strategy_used} ({result.time_taken:.3f}s)")
                else:
                    logger.error(f"❌ Injection #{self.injection_count} failed: {result.error_message}")
    ...
    def _update_stats(self, result: InjectionResult, total_time: float):
        ...
        self.performance_stats['average_injection_time'] = \
            (prev_avg * (total_inj - 1) + total_time) / total_inj
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_injection_time_ms': round(self.performance_stats['average_injection_time'] * 1000, 1),
