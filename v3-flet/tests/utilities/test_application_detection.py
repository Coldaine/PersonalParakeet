...
def test_application_detection():
    ...
    for i in range(5):
        ...
        if app_info:
            print(f"  {i+1}. {app_info.name} ({app_info.app_type.name}) - '{app_info.window_title[:50]}...'")
            
            # Get application profile
            profile = detector.get_application_profile(app_info)
            print(f"     Strategy order: {profile.preferred_strategies}")
            print(f"     Performance: key_delay={profile.key_delay}ms, supports_paste={profile.supports_paste}")
        else:
...
def test_injection_manager_integration():
    ...
    if app_info:
        ...
        print(f"  Profile settings: key_delay={profile.key_delay}, focus_delay={profile.focus_delay}")
...
def interactive_detection_test():
    ...
    while True:
        ...
        if app_info:
            ...
            print(f"  ⏱️  Timing: key_delay={profile.key_delay}ms, focus_delay={profile.focus_delay}ms")
            print()
        else:
...
def main():
    ...
    try:
        ...
        response = input("This lets you switch between apps and see real-time detection (y/n): ").strip()
